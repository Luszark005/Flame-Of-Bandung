...
public class ProjectileManager {
	...
	private void importImgs() {
		...
		projImgs[1] = bullet.getSubimage(0, 0, TILE_SIZE, TILE_SIZE);
	...
	public void newProj(Pejuang p, Penjajah ph) {
		...
		for(Projectile pr : projectiles) {
			if(!pr.isActive()) {
				if(pr.getProjectileType() == type) {
					pr.reuse(p.getX() + TILE_SIZE/2, p.getY() + TILE_SIZE/2, xSpeed, ySpeed, p.getDamage(), rotate);
					return;
				}
			}
		}
	...
	public void draw(Graphics g) {
		...
		for(Projectile pr : projectiles)
			if(pr.isActive()) {
				...
				g2d.drawImage(projImgs[1], -TILE_SIZE/2, -TILE_SIZE/2, null);
				g2d.rotate(-Math.toRadians(pr.getRotation()));
				g2d.translate(-pr.getPos().x, -pr.getPos().y);
...
//		System.out.println("Size of proj:" + projectiles.size());
	}
	
	private boolean isProjOutOfBounds(Projectile pr) {
		return (pr.getPos().x < 0 && pr.getPos().y < 0 && pr.getPos().y > GAME_HEIGHT && pr.getPos().x > ACTIONBAR_XSTART);
	}
